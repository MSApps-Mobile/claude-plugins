...
5. **Per-card decision — for every blocked card:**
   ...
   d. **Decide and act:**
      ...
      - **Route to Backlog** — gate still active (any reason — time, partner, operator cred-mint, marketplace decision, ToS, lawyer signoff, anything). Move the card to `🧊 Backlog` (create the column via `["api","-X","POST","--data","{\"name\":\"🧊 Backlog\",\"idBoard\":\"<boardId>\",\"pos\":\"top\"}","lists"]` if it doesn't exist), comment with the current gate state + explicit unblock command (one line: "Gate: SENDGRID_API_KEY still sentinel as of <ts>; unblock: paste real key + gcloud secrets versions add"). Add to the recap's "needs Michal" section if a human step is required to clear it. Per `feedback_blocked_pipeline_must_drain` + rule card [MyWi01lz](https://trello.com/c/MyWi01lz) (2026-05-29): **🚧 Blocked column empty at end of run is the hard acceptance criterion — NO exceptions.** The prior "operator-gated cred-mint stays in Blocked" carve-out is retired; cred-mint gates also route to Backlog.
